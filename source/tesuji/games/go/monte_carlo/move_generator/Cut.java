...
public class Cut extends AbstractMoveGenerator
{
	...
	public int generate()
	{
		...
		for (int n=0; n<4; n++)
			if (isCut(FourCursor.getNeighbour(xy, n),oppositeColor))
				administration.getProbabilityMap().add(xy, getWeight());
	...
	private boolean isCut(int xy, byte color)
	{
		...
		if (xy==0 || blackNeighbours[xy]==0 || whiteNeighbours[xy]==0)
			return false;
