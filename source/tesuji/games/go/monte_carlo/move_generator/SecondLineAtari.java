...
public class SecondLineAtari extends LadderMoveGenerator
{
//	@Override
	public int generate()
	{
...
//					if (isSafeToMove(neighbour))
	...
	private boolean isBlock(int xy, byte color)
	{
		...
		if (board[xy]!=EMPTY || neighbours[xy]>2)
			return false;

		byte oppositeColor = opposite(color);
		
		if (administration.isSecondRow(left(xy)) && board[left(xy)]==oppositeColor && liberties[chain[left(xy)]]==2)
			return true;
		if (administration.isSecondRow(right(xy)) && board[right(xy)]==oppositeColor && liberties[chain[right(xy)]]==2)
			return true;
		if (administration.isSecondRow(above(xy)) && board[above(xy)]==oppositeColor && liberties[chain[above(xy)]]==2)
			return true;
		if (administration.isSecondRow(below(xy)) && board[below(xy)]==oppositeColor && liberties[chain[below(xy)]]==2)
			return true;
